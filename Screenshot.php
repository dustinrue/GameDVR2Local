...
Class Screenshot {
  ...
  public function Screenshot($clip_data = null, $output_filename = null) {
  ...
  public function download() {
    ...
    $game_clip_uris = $this->clip_data->screenshotUris;
    foreach ($game_clip_uris AS $game_clip_uri) {
        ...
        if ($game_clip_uri->uriType == "Download") {
          ...
          if (!file_exists($filename)) {
            printf("Downloading \"%s\"...", ($this->clip_data->userCaption != "") ? $this->clip_data->userCaption:$this->clip_data->screenshotId);
            ...
            fclose($source_file);
            fclose($output_file);
            touch($filename, strtotime($this->clip_data->dateTaken));
            echo "done\n";
          }
          else {
            touch($filename, strtotime($this->clip_data->dateTaken));
            printf("\"%s\" already exists\n", ($this->clip_data->userCaption != "") ? $this->clip_data->userCaption:$this->clip_data->screenshotId);
          }
        }
  ...
  private function generate_filename() {
    ...
    if ($this->clip_data->userCaption != "") {
      $filename = $this->output_filename . DIRECTORY_SEPARATOR . $this->gt . DIRECTORY_SEPARATOR . $this->clip_data->titleName . DIRECTORY_SEPARATOR . $this->clip_data->userCaption . " (" . $this->clip_data->screenshotId . ")" . ".png";
    }
    else {
      $filename = $this->output_filename . DIRECTORY_SEPARATOR . $this->gt . DIRECTORY_SEPARATOR . $this->clip_data->titleName . DIRECTORY_SEPARATOR . $this->clip_data->screenshotId  . ".png";
    }
